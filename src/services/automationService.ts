...
class AutomationService {
  ...
  async mergeTickets(sourceTicketId: string, targetTicketId: string, mergeReason: string, userId: string): Promise<void> {
    try {
      ...
      await supabase
        ...
        .insert({
          ...
          action_type: 'auto_assign', // Using existing enum value
          action_details: {
            action: 'ticket_merge',
            target_ticket_id: targetTicketId,
            merged_by: userId,
      ...
      console.log(`Ticket ${sourceTicketId} merged into ${targetTicketId}`);
    } catch (error) {
  ...
  // Improved auto-assignment with skill matching
  async smartAutoAssignTicket(ticketId: string, categoryName: string, ticketDescription: string): Promise<string | null> {
    try {
      const { data, error } = await supabase.rpc('smart_auto_assign_ticket', {
        ticket_id: ticketId,
